import pg from 'pg';
import { standardsSeed, industriesSeed, categoriesSeed, productsSeed } from './server/seed-data.ts';
...
async function seedDatabase() {
  try {
    ...
    console.log(`✓ Seeded ${standardsSeed.length} standards`);
    
    // Seed Industries/Applications
    console.log("🏭 Seeding Industries...");
    for (const industry of industriesSeed) {
      const applicationsJson = JSON.stringify(industry.applications);
      await pool.query(
    ...
    console.log(`✓ Seeded ${industriesSeed.length} industries`);
