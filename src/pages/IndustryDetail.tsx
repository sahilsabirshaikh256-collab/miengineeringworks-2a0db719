...
const IndustryDetail = () => {
  ...
  return (
    <PageTransition>
      <Helmet>
        <title>{`${industry.name} Fasteners - ASTM A193 B7 | M.I. Engineering Works`}</title>
        <meta name="description" content={`ASTM A193 Grade B7 fasteners for ${industry.name}. ${industry.description} Manufacturer & supplier in Mumbai, India.`} />
        <meta name="keywords" content={`ASTM A193 B7 ${industry.name} fasteners, ${industry.name} bolts, ${industry.name} stud bolts, industrial fasteners Mumbai`} />
        ...
        <div className="container">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {industry.applications.map((app, i) => (
              <motion.div
                key={app.name}
                className="group relative rounded-lg overflow-hidden shadow-md hover:shadow-gold transition-all"
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {otherIndustries.map((ind) => (
              <Link
                ...
                className="group relative rounded-lg overflow-hidden aspect-[4/3] shadow-md hover:shadow-gold transition-all"
              >
                <img src={ind.image} alt={ind.name} loading="lazy" className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent" />
                <div className="absolute bottom-0 p-3">
                  <h3 className="text-white text-xs md:text-sm font-semibold">{ind.name}</h3>
                  <span className="inline-flex items-center gap-1 text-primary text-[10px] mt-1 opacity-0 group-hover:opacity-100 transition-opacity">
